...
pl_experiment = mcc.get_experiments(cre=False, 
...
structure_unionizes = mcc.get_structure_unionizes([pl_exp_ids[0]], 
                                                  is_injection=False,
                                                  hemisphere_ids=[1, 2],
                                                  include_descendants=True).reset_index(drop=True)

proj = structure_unionizes.sort_values(by=["projection_energy"], ascending=True).reset_index(drop=True)
...
cfos = pd.read_csv(r"C:\Users\shane\Dropbox (ListonLab)\shane\python_projects\get_atlas_proj_density\data\Cohort6_cfos_6_22_21_combined_results - all_density_excl.csv")
...
for row in combined.iterrows():
    t_stat, p_val = ttest_ind(
        [row[1].ChR2_SJ0619, row[1].ChR2_SJ0602, row[1].ChR2_SJ0603, row[1].ChR2_SJ0605, row[1].ChR2_SJ0612],
        [row[1].YFP_SJ0601, row[1].YFP_SJ0604, row[1].YFP_SJ0606, row[1].YFP_SJ0610, row[1].YFP_SJ0613, row[1].YFP_SJ0615],
        axis=0, equal_var=True, nan_policy='propagate', permutations=None, random_state=None, alternative='two-sided', trim=0)
...
plt.scatter(combined["t_stat"], np.log(combined["projection_density"]+0.0001))


plt.scatter(combined_pl_proj["t_stat"], np.log(combined_pl_proj["projection_density"]))
plt.show()
...
sns.histplot(data=combined, x="pd_log", bins=100)
...
sns.histplot(data=combined_pl_proj, x="pd_log", bins=100)
plt.show()


sns.scatterplot(data=combined_pl_proj, x="pd_log", y="effect_size")
plt.show()


sns.scatterplot(data=combined_pl_proj, x="pd_log", y="t_stat")
plt.show()
